...
if [ -f "${SCRIPT_DIR}/docs/versions.env" ]; then
    # shellcheck disable=SC1091
    source "${SCRIPT_DIR}/docs/versions.env"
fi
...
rollback_on_failure() {
    ...
    if [ "${PG_UPDATE_PHASE:-}" = "deploy" ] && [ -n "${PRE_PULL_COMMIT:-}" ] && [ "${exit_code}" -ne 0 ]; then
        echo ""
        echo "ERROR: Update failed (exit ${exit_code}). Rolling back..."
        git reset --hard "${PRE_PULL_COMMIT}" 2>/dev/null || \
            echo "WARNING: git rollback failed. Manual recovery: git reset --hard ${PRE_PULL_COMMIT}"
        # Restore /opt/pg-monitor from snapshot taken before deploy
        if [ -n "${PG_MONITOR_SNAPSHOT:-}" ] && [ -d "${PG_MONITOR_SNAPSHOT}" ]; then
            echo "Restoring /opt/pg-monitor from snapshot..."
            rm -rf /opt/pg-monitor
            cp -r "${PG_MONITOR_SNAPSHOT}" /opt/pg-monitor 2>/dev/null || \
                echo "WARNING: /opt/pg-monitor restore failed. Snapshot at: ${PG_MONITOR_SNAPSHOT}"
        fi
    fi
    exit "${exit_code}"
}
...
if ! command -v patroni &>/dev/null; then
    ...
    /opt/patroni/bin/pip install -r "${SCRIPT_DIR}/docs/requirements.txt" >/dev/null 2>&1
...
# Snapshot /opt/pg-monitor before overwriting so rollback_on_failure can restore it
PG_MONITOR_SNAPSHOT=""
if [ -d /opt/pg-monitor ]; then
    PG_MONITOR_SNAPSHOT="/tmp/pg-monitor-snap-$(date +%s)"
    cp -r /opt/pg-monitor "${PG_MONITOR_SNAPSHOT}" 2>/dev/null || PG_MONITOR_SNAPSHOT=""
fi
export PG_MONITOR_SNAPSHOT
...
if [ -d /opt/pg-backup ]; then
    cp scripts/backup-run.sh /opt/pg-backup/backup-run.sh 2>/dev/null || true
    chmod +x /opt/pg-backup/backup-run.sh 2>/dev/null || true
fi
...
# Build the React frontend from source
...
# Always regenerate sudoers to pick up permission changes from updates
if [ -f /opt/pg-monitor/scripts/cluster-monitor.sh ]; then
    bash /opt/pg-monitor/scripts/cluster-monitor.sh --sudoers-only 2>/dev/null || true
fi
