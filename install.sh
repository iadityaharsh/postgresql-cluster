...
if [ -d "${DIR}" ]; then
    ...
    if [ -f "cluster.conf" ]; then
        ...
        if ! bash -c 'source scripts/cluster-common.sh && load_config' &>/dev/null; then
            echo ""
            echo "WARNING: cluster.conf failed validation. Details:"
            bash -c 'source scripts/cluster-common.sh && load_config' || true
            echo ""
        fi
        ...
        if ! grep -q '^PATRONI_API_USER=' cluster.conf 2>/dev/null; then
            PATRONI_API_PASS_GEN=$(openssl rand -base64 24 | tr -d '/+=' | head -c 24)
            {
                ...
                echo 'PATRONI_API_USER="patroni"'
                echo "PATRONI_API_PASS=\"${PATRONI_API_PASS_GEN}\""
            } >> cluster.conf
        ...
        if ! grep -q '^INTERNAL_SECRET=' cluster.conf 2>/dev/null; then
            ...
            {
                echo ""
                echo "# --- Internal node-to-node auth ---"
                echo "INTERNAL_SECRET=\"${INTERNAL_SECRET_GEN}\""
            } >> cluster.conf
        ...
        if [[ "${reconfigure,,}" == "y" ]]; then
            ...
            echo "  sudo ./scripts/cluster-setup.sh"
        fi
    else
