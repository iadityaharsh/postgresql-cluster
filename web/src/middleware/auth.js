...
const CSRF_EXEMPT_PATHS = ['/api/login', '/api/logout', '/api/auth/status', '/api/auth/change-password'];
const MUTATING_METHODS = new Set(['POST', 'PUT', 'DELETE', 'PATCH']);

function authMiddleware(req, res, next) {
  // No auth configured — allow everything (install/setup flow)
  if (!authConfig) return next();
  // Internal node-to-node paths: validated by X-Internal-Token, no CSRF needed
  ...
  // CSRF check for mutating requests (Double Submit Cookie pattern)
  if (MUTATING_METHODS.has(req.method) && !pathMatches(CSRF_EXEMPT_PATHS, req.path)) {
    const sessionCsrf = sessions.get(token)?.csrf;
...
// Clean up expired sessions every hour (.unref so tests don't hang)
setInterval(() => {
