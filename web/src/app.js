...
function createApp() {
  ...
  const ctx = { nodes, conf, confPath, fetchJSON, CLUSTER_NAME, VIP, PG_PORT, PG_PASS, PORT, PATRONI_API_USER, PATRONI_API_PASS, findScript, reloadConf, updateConfKeys };
  ...
  app.post('/api/login', async (req, res) => {
    ...
    if (username !== currentAuth.username) return res.status(401).json({ error: 'Invalid credentials' });
  ...
  // Mount routers
  app.use('/api', createClusterRouter(ctx));
