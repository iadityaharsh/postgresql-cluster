              ...
              <table>
                ...
                <tbody>
                  {backups.archives.map((a, i) => (
                    <tr key={i}>
                      <td>
                        <span className="backup-name">{a.name}</span>
                        {backups.encryption && (
                          <span style={{
                            marginLeft: 8, fontSize: 10, fontWeight: 600, padding: '1px 6px',
                            borderRadius: 4, verticalAlign: 'middle',
                            background: backups.encryption.encrypted ? 'var(--success-bg, #1a3a2a)' : 'var(--warn-bg, #3a2a1a)',
                            color: backups.encryption.encrypted ? 'var(--success, #4caf50)' : 'var(--warn, #ff9800)',
                          }}>
                            {backups.encryption.encrypted ? '🔒 Encrypted' : '⚠ Unencrypted'}
                          </span>
                        )}
                      </td>
