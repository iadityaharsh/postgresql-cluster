...
export default function SettingsTab() {
  ...
  const [joinForm, setJoinForm] = useState({ node_ip: '', node_name: '', node_number: '' });
  ...
  const uploadConfigBackup = async (e) => {
    const file = e.target.files[0];
    e.target.value = '';
    if (!file) return;
    if (!file.name.endsWith('.tar.gz')) {
      return setToast({ message: 'Please select a .tar.gz backup file', type: 'error' });
    }
    setRestoring(true);
    try {
      ...
      if (!resp.ok) {
        setToast({ message: data.error || 'Restore failed', type: 'error' });
        setRestoring(false);
        return;
      }
      setRestoreStatus('reconnecting');
      const delay = (data.delay || 10) * 1000;
      ...
      const poll = setInterval(async () => {
        ...
        try {
          ...
          if (attempts > 30) {
            clearInterval(poll);
            setToast({ message: 'Server not responding after restore — try refreshing manually', type: 'error' });
            setRestoring(false);
  ...
  const restartTunnel = async () => {
    ...
    try {
      ...
      const d = await resp.json();
      const ok = (d.results || []).filter(r => r.status === 'restarted').length;
      const fail = (d.results || []).filter(r => r.status !== 'restarted').length;
      setToast({ message: `Tunnel restarted on ${ok} node${ok !== 1 ? 's' : ''}${fail ? `, ${fail} failed` : ''}`, type: fail ? 'error' : 'ok' });
      setTimeout(fetchTunnel, 2000);
    } catch { setToast({ message: 'Failed to restart tunnel', type: 'error' }); }
        ...
        <div className="panel">
          ...
          {tunnelOpen && (
            <div className="panel-body">
              ...
              {tunnelNodes.some(n => n.installed) && (
                <div style={{ marginBottom: 14 }}>
                  <button className="btn" style={{ fontSize: 12 }} onClick={restartTunnel} disabled={restartingTunnel}>
                    {restartingTunnel ? 'Restarting...' : 'Restart Tunnel on All Nodes'}
                  </button>
                </div>
              )}
              {!tunnel?.configured && (
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
            Single-file backup containing full cluster config and credentials. Keep this file secure — it can restore the entire cluster.
          </div>
          {restoreStatus === 'reconnecting' ? (
            <div style={{ fontSize: 13, color: 'var(--warn)' }}>
              Config restored — reconnecting…
            </div>
          ) : (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button className="btn primary" onClick={downloadConfigBackup} disabled={restoring}>
                Download Config Backup
              </button>
              <button className="btn" onClick={() => restoreInputRef.current?.click()} disabled={restoring}>
                {restoring ? 'Restoring…' : 'Upload Config Backup'}
              </button>
              <input
                ref={restoreInputRef}
                type="file"
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12, lineHeight: 1.5 }}>
            Generate a join config for adding a new node to this cluster.
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <input
              type="text"
              placeholder="New node IP (e.g. 10.0.0.14)"
              value={joinForm.node_ip}
              onChange={e => setJoinForm(f => ({ ...f, node_ip: e.target.value }))}
              style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text)', fontSize: 13 }}
            />
            <input
              type="text"
              placeholder="Node name (e.g. node-04)"
              value={joinForm.node_name}
              onChange={e => setJoinForm(f => ({ ...f, node_name: e.target.value }))}
              style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text)', fontSize: 13 }}
            />
            <input
              type="number"
              placeholder="Node number (e.g. 4)"
              value={joinForm.node_number}
              onChange={e => setJoinForm(f => ({ ...f, node_number: e.target.value }))}
              style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg-input)', color: 'var(--text)', fontSize: 13 }}
            />
            <button
              className="btn"
              disabled={!joinForm.node_ip || !joinForm.node_name || !joinForm.node_number}
              onClick={() => {
                const params = new URLSearchParams(joinForm).toString();
                window.open(`/api/config/join-config?${params}`, '_blank');
              }}
            >
