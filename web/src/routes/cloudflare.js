...
module.exports = function createCloudflareRouter(ctx) {
  ...
  const { nodes, conf, PORT, VIP, PG_PORT, PG_PASS, CLUSTER_NAME, findScript, updateConfKeys, pool, SELF_PROTO, internalLib, internalOpts } = ctx;
  ...
  router.get('/tunnel/local', (req, res) => {
    ...
    if (!result.running) { try { result.running = require('child_process').execSync('pgrep -x cloudflared >/dev/null 2>&1 && echo yes').toString().trim() === 'yes'; } catch {} }
    res.json(result);
  });

  // Restart cloudflared on this node
  router.post('/tunnel/restart/local', (req, res) => {
    try {
      require('child_process').execSync('sudo systemctl restart cloudflared', { timeout: 30000 });
      res.json({ status: 'restarted' });
    } catch (err) { res.json({ status: 'failed', error: err.message }); }
  });

  // Restart cloudflared on all nodes
    ...
    for (const node of nodes) {
      try {
        const data = await new Promise((resolve, reject) => {
          const postData = '';
          const r = internalLib.request({ hostname: node.ip, port: PORT, path: '/api/tunnel/restart/local', method: 'POST', timeout: 35000, ...internalOpts, headers: { 'Content-Type': 'application/json', 'X-Internal-Token': conf.INTERNAL_SECRET || '' } }, (resp) => {
            let body = '';
            resp.on('data', d => body += d);
            resp.on('end', () => { try { resolve(JSON.parse(body)); } catch { resolve({ status: 'unknown' }); } });
          });
          r.on('error', (e) => resolve({ status: 'failed', error: e.message }));
          r.on('timeout', () => { r.destroy(); resolve({ status: 'failed', error: 'timeout' }); });
          r.end();
        });
        results.push({ name: node.name, ...data });
      } catch (err) { results.push({ name: node.name, status: 'failed', error: err.message }); }
    }
    res.json({ results });
  });
  ...
  router.get('/tunnel', async (req, res) => {
    const results = await Promise.all(
      nodes.map(async node => {
        const data = await ctx.fetchJSON(`${SELF_PROTO}://${node.ip}:${PORT}/api/tunnel/local`);
  ...
  async function deployTunnelToNodes(token, task) {
    const scriptPath = findScript('cloudflare-tunnel.sh');
    if (!scriptPath) { task.log.push(`[${new Date().toLocaleTimeString()}] ERROR: cloudflare-tunnel.sh not found`); return false; }
    ...
    for (const node of nodes.filter(n => !myIps.has(n.ip))) {
      ...
      await new Promise((resolve) => {
        ...
        const r = internalLib.request({ hostname: node.ip, port: PORT, path: '/api/tunnel/apply', method: 'POST', ...internalOpts, headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(postData), 'X-Internal-Token': conf.INTERNAL_SECRET || '' }, timeout: 120000 }, (resp) => {
          let body = '';
          resp.on('data', d => body += d);
  ...
  router.post('/tunnel/setup', async (req, res) => {
    ...
    const scriptPath = findScript('cloudflare-tunnel.sh');
    if (!scriptPath) return res.status(404).json({ error: 'cloudflare-tunnel.sh not found' });
  ...
  router.post('/hyperdrive/create-user', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        await dbClient.query(`GRANT USAGE ON SCHEMA public TO ${safeUser}`);
      ...
      // Ensure pg_hba allows localhost connections (needed for Cloudflare Tunnel)
      const localhostRule = 'host all all 127.0.0.1/32 scram-sha-256';
      try {
        const pAuthObj = PATRONI_API_USER ? { user: PATRONI_API_USER, pass: PATRONI_API_PASS } : undefined;
        const patroniCfg = await ctx.fetchJSON(`https://${nodes[0].ip}:8008/config`, 5000, pAuthObj);
        if (patroniCfg?.postgresql?.pg_hba && !patroniCfg.postgresql.pg_hba.some(r => r.includes('127.0.0.1/32'))) {
          const pgHba = [...patroniCfg.postgresql.pg_hba];
          // Insert before the catch-all hostssl rule
          const catchAllIdx = pgHba.findIndex(r => r.startsWith('hostssl all all 0.0.0.0/0'));
          if (catchAllIdx >= 0) pgHba.splice(catchAllIdx, 0, localhostRule);
          else pgHba.push(localhostRule);
          const patchData = JSON.stringify({ postgresql: { pg_hba: pgHba } });
          const patchHeaders = { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(patchData) };
          if (PATRONI_API_USER) patchHeaders['Authorization'] = 'Basic ' + Buffer.from(`${PATRONI_API_USER}:${PATRONI_API_PASS}`).toString('base64');
          await new Promise((resolve) => {
            const r = https.request({ hostname: nodes[0].ip, port: 8008, path: '/config', method: 'PATCH', headers: patchHeaders, timeout: 10000, rejectUnauthorized: false }, (resp) => {
              let body = ''; resp.on('data', d => body += d); resp.on('end', () => resolve(body));
            });
            r.on('error', () => resolve(null));
            r.on('timeout', () => { r.destroy(); resolve(null); });
            r.write(patchData); r.end();
          });
        }
      } catch {}
  ...
  async function dropUserOnFailure(username, database, task, ts) {
    try {
      const client = await pool.connect();
      try {
        const safeUser = client.escapeIdentifier(username);
        ...
        const exists = await client.query('SELECT 1 FROM pg_roles WHERE rolname=$1', [username]);
        if (exists.rows.length > 0) {
          await client.query(`REVOKE ALL PRIVILEGES ON DATABASE ${safeDb} FROM ${safeUser}`);
          await client.query(`DROP ROLE ${safeUser}`);
          task.log.push(`${ts()} Cleaned up DB user "${username}"`);
        }
      } finally { client.release(); }
    } catch (err) { task.log.push(`${ts()} Warning: could not clean up user "${username}": ${err.message}`); }
  }
  ...
  router.post('/hyperdrive', async (req, res) => {
    ...
    res.json({ status: 'started' });
    try {
      // Ensure the DB user exists with the current password before verifying
      hyperdriveTask.log.push(`${ts()} Ensuring DB user "${username}" exists...`);
      try {
        const setupClient = await pool.connect();
        try {
          const safeUser = setupClient.escapeIdentifier(username);
          const safeDb = setupClient.escapeIdentifier(database);
          const safePw = setupClient.escapeLiteral(password);
          const exists = await setupClient.query('SELECT 1 FROM pg_roles WHERE rolname=$1', [username]);
          if (exists.rows.length > 0) {
            await setupClient.query(`ALTER USER ${safeUser} WITH PASSWORD ${safePw}`);
          } else {
            await setupClient.query(`CREATE USER ${safeUser} WITH PASSWORD ${safePw}`);
          }
          await setupClient.query(`GRANT ALL PRIVILEGES ON DATABASE ${safeDb} TO ${safeUser}`);
        } finally { setupClient.release(); }
        const dbPool2 = new Pool({ host: VIP || nodes[0].ip, port: parseInt(PG_PORT), user: 'postgres', password: PG_PASS, database, connectionTimeoutMillis: 5000 });
        const dbClient2 = await dbPool2.connect();
        try {
          const safeUser = dbClient2.escapeIdentifier(username);
          await dbClient2.query(`GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${safeUser}`);
          await dbClient2.query(`GRANT USAGE ON SCHEMA public TO ${safeUser}`);
          await dbClient2.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON TABLES TO ${safeUser}`);
        } finally { dbClient2.release(); await dbPool2.end().catch(() => {}); }
        hyperdriveTask.log.push(`${ts()} DB user ready`);
      } catch (setupErr) {
        hyperdriveTask.log.push(`${ts()} ERROR: Failed to set up DB user — ${setupErr.message}`);
        hyperdriveTask.exitCode = 1; hyperdriveTask.running = false;
        return;
      }

      // Verify the DB user can actually connect before calling Cloudflare
      hyperdriveTask.log.push(`${ts()} Verifying database credentials locally...`);
      const testPool = new Pool({ host: VIP || nodes[0].ip, port: parseInt(PG_PORT), user: username, password, database, connectionTimeoutMillis: 5000 });
      try {
        const testClient = await testPool.connect();
        await testClient.query('SELECT 1');
        testClient.release();
        hyperdriveTask.log.push(`${ts()} Local connection OK — user "${username}" can connect to "${database}"`);
      } catch (dbErr) {
        hyperdriveTask.log.push(`${ts()} ERROR: Local connection failed — ${dbErr.message}`);
        hyperdriveTask.log.push(`${ts()} The DB user cannot connect. Did you click "Create DB User" first?`);
        hyperdriveTask.log.push(`${ts()} If you did, check pg_hba.conf allows the connection.`);
        await dropUserOnFailure(username, database, hyperdriveTask, ts);
        hyperdriveTask.exitCode = 1; hyperdriveTask.running = false;
      ...
      else { origin.port = parseInt(PG_PORT) || 5432; }
      hyperdriveTask.log.push(`${ts()} Sending config to Cloudflare API...`);
  ...
  router.put('/hyperdrive/:key', async (req, res) => {
    ...
    if (cfg.access_client_id) { origin.access_client_id = cfg.access_client_id; }
    else { origin.port = parseInt(PG_PORT) || 5432; }
