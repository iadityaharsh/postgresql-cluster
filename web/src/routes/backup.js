...
module.exports = function createBackupRouter(ctx) {
  ...
  const BACKUP_SCRIPT = '/opt/pg-backup/backup-run.sh';

  function runBorg(args, timeout = 15000) {
    return new Promise((resolve, reject) => {
      const env = {
        ...process.env,
        BORG_PASSPHRASE: conf.BORG_PASSPHRASE || '',
        BORG_REPO,
        BORG_UNKNOWN_UNENCRYPTED_REPO_ACCESS_IS_OK: 'yes',
        BORG_RELOCATED_REPO_ACCESS_IS_OK: 'yes',
      };
      execFile('sudo', ['-E', 'borg', ...args], { env, timeout, stdin: 'ignore' }, (err, stdout, stderr) => {
  ...
  router.get('/', async (req, res) => {
    try {
      const [listOut, infoOut] = await Promise.all([
        runBorg(['list', '--json', BORG_REPO]),
        runBorg(['info', '--json', BORG_REPO]).catch(() => null),
      ]);
      ...
      let encryption = { mode: 'unknown', encrypted: false };
      try {
        const info = JSON.parse(infoOut);
        const mode = info.encryption?.mode || 'unknown';
        encryption = { mode, encrypted: mode !== 'none' && mode !== 'unknown' };
      } catch {}
      res.json({ available: true, archives, encryption });
    } catch (err) {
      if (err.message.includes('No such file') || err.message.includes('not a valid')) {
        res.json({ available: false, archives: [], error: 'Borg repo not found. Run backup-setup.sh first.' });
      } else {
  ...
  router.post('/', (req, res) => {
    ...
    if (!fs.existsSync(BACKUP_SCRIPT)) return res.status(404).json({ error: 'Backup script not found. Run backup-setup.sh first.' });
  ...
  router.post('/restore', (req, res) => {
    ...
    (async () => {
      const env = { ...process.env, BORG_PASSPHRASE: conf.BORG_PASSPHRASE || '', PGPASSWORD: PG_PASS, BORG_UNKNOWN_UNENCRYPTED_REPO_ACCESS_IS_OK: 'yes', BORG_RELOCATED_REPO_ACCESS_IS_OK: 'yes' };
  ...
  router.get('/storage', (req, res) => {
    try {
      let mounted = false;
      try {
        if (fs.existsSync('/mnt/pg-backup')) {
