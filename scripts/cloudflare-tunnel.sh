...
# Usage: ./cloudflare-tunnel.sh <token>
# ================================================================
...
if [ -z "${TOKEN}" ]; then
    ...
    echo "     ./cloudflare-tunnel.sh <token>"
    exit 1
fi
...
# Step 3: Write token to a mode-600 EnvironmentFile and install a systemd unit.
# Never use `cloudflared service install ${TOKEN}` — that embeds the token in ExecStart
# which is world-readable via /etc/systemd/system/.
echo "Installing cloudflared service (token stored in /etc/cloudflared/tunnel-env)..."
