...
# Run on each node: sudo ./scripts/cluster-setup.sh
# ================================================================
...
# ---- Check for join mode BEFORE load_config ----
# Join mode: join-config.json exists (placed here by configure.sh or scp'd from primary).
# On a fresh joining node, cluster.conf does not exist yet — we generate it from join-config.json.
JOIN_CONFIG="${SCRIPT_DIR}/../join-config.json"
JOIN_MODE=false
if [ -f "${JOIN_CONFIG}" ]; then
    echo "  Found join-config.json — running in replica-join mode."
    JOIN_MODE=true
    JOIN_THIS_NODE=$(python3 -c "import json; d=json.load(open('${JOIN_CONFIG}')); print(d['this_node'])" 2>/dev/null)
    JOIN_THIS_NAME=$(python3 -c "import json; d=json.load(open('${JOIN_CONFIG}')); print(d['this_node_name'])" 2>/dev/null)
    JOIN_THIS_IP=$(python3 -c "import json; d=json.load(open('${JOIN_CONFIG}')); print(d['this_node_ip'])" 2>/dev/null)
    if [ -z "${JOIN_THIS_NODE:-}" ] || [ -z "${JOIN_THIS_NAME:-}" ] || [ -z "${JOIN_THIS_IP:-}" ]; then
        echo "ERROR: join-config.json is missing required fields (this_node, this_node_name, this_node_ip)." >&2
        exit 1
    fi
    # Generate cluster.conf from join-config.json if it doesn't exist
    if [ ! -f "${CONF_FILE}" ]; then
        echo "  Generating cluster.conf from join-config.json..."
        python3 - "${JOIN_CONFIG}" "${CONF_FILE}" <<'PYEOF'
import json, sys, os
jc = json.load(open(sys.argv[1]))
out = sys.argv[2]
peers = jc.get('etcd_peers', [])
node_count = max(int(jc.get('this_node', 1)), len(peers))
lines = [
    f'CLUSTER_NAME="{jc["cluster_name"]}"',
    f'NODE_COUNT="{node_count}"',
    f'NODE_{jc["this_node"]}_NAME="{jc["this_node_name"]}"',
    f'NODE_{jc["this_node"]}_IP="{jc["this_node_ip"]}"',
]
for i, ip in enumerate(peers, 1):
    lines.append(f'NODE_{i}_IP="{ip}"')
for k in ['PG_VERSION','PG_PORT','PG_DATA_DIR','PG_BIN_DIR','PG_MAX_CONN',
          'PG_HBA_SUBNET','PG_SUPERUSER_PASS','PG_REPLICATOR_PASS','PG_ADMIN_PASS',
          'PATRONI_API_USER','PATRONI_API_PASS','ETCD_TOKEN','ENABLE_VIP',
          'VIP_ADDRESS','VIP_NETMASK','VIP_INTERFACE','INTERNAL_SECRET',
          'MONITOR_PORT','PG_ADMIN_PASS']:
    v = jc.get(k.lower(), jc.get(k, ''))
    if v:
        lines.append(f'{k}="{v}"')
# Ensure required fields have defaults
defaults = {'PG_VERSION':'17','PG_PORT':'5432','PG_MAX_CONN':'200',
            'ENABLE_VIP':'false','PATRONI_API_USER':'patroni','MONITOR_PORT':'8080'}
existing_keys = {l.split('=')[0] for l in lines}
...
with open(out, 'w') as f:
    f.write('\n'.join(lines) + '\n')
os.chmod(out, 0o600)
print(f'  Generated {out} with {len(lines)} entries.')
PYEOF
    fi
fi
...
if [ "${JOIN_MODE}" = true ]; then
    NODE_NUM="${JOIN_THIS_NODE}"
    NODE_NAME="${JOIN_THIS_NAME}"
    NODE_IP="${JOIN_THIS_IP}"
else
...
echo "  IP: ${NODE_IP}"
if [ "${JOIN_MODE}" = false ] && [ "$NODE_NUM" -eq 1 ]; then
...
if [ ! -f "$ETCD_SSL_DIR/ca.crt" ]; then
    if [ "${JOIN_MODE}" = false ] && [ "$NODE_NUM" -eq 1 ]; then
        ...
        echo "  scp root@<NODE1_IP>:$ETCD_SSL_DIR/ca.crt $ETCD_SSL_DIR/ca.crt" >&2
        echo "Do NOT copy ca.key to non-Node-1 nodes." >&2
        exit 1
    fi
fi

# Generate server and peer certs signed by CA (requires ca.key — Node 1 only)
...
if [ "$PATRONI_HEALTHY" = true ] && [ "$FORCE" != true ]; then
    ...
    if [ "${JOIN_MODE}" = false ] && [ "$NODE_NUM" -eq 1 ] && [ -d "${PG_DATA_DIR}" ] && [ -n "$(ls -A "${PG_DATA_DIR}" 2>/dev/null)" ]; then
        echo ""
...
if [ "$REGEN_CERT" = true ]; then
    echo "Generating self-signed SSL certificate for PostgreSQL (1yr, with SAN)..."
    umask 077
    openssl req -new -x509 -days 365 -nodes \
        -out "$SSL_DIR/server.crt" \
        -keyout "$SSL_DIR/server.key" \
        -subj "/CN=${NODE_NAME}/O=${CLUSTER_NAME}" \
        -addext "subjectAltName=DNS:${NODE_NAME},IP:${NODE_IP},IP:127.0.0.1" \
        2>/dev/null
...
echo "=============================================="
bash "${SCRIPT_DIR}/cluster-monitor.sh"
echo ""
