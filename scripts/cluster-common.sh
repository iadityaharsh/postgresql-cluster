...
validate_config() {
    ...
    local required=(
        ...
        PG_SUPERUSER_PASS PG_REPLICATOR_PASS PG_ADMIN_PASS PATRONI_API_PASS
    ...
    # VIP_NETMASK is optional (defaults to 24) but if set must be a valid host prefix (8-30)
    if [ -n "${VIP_NETMASK:-}" ] && ! [[ "${VIP_NETMASK}" =~ ^[0-9]+$ && "${VIP_NETMASK}" -ge 8 && "${VIP_NETMASK}" -le 30 ]]; then
        echo "ERROR: VIP_NETMASK must be an integer 8-30, got '${VIP_NETMASK}'" >&2
        errors=$((errors + 1))
    fi

    # PG_HBA_SUBNET must be a valid CIDR notation
    if [ -n "${PG_HBA_SUBNET:-}" ] && ! python3 -c "import ipaddress,sys; ipaddress.ip_network(sys.argv[1],strict=False)" "${PG_HBA_SUBNET}" 2>/dev/null; then
        echo "ERROR: PG_HBA_SUBNET='${PG_HBA_SUBNET}' is not valid CIDR notation" >&2
        errors=$((errors + 1))
    fi
...
get_node_ip() { local _v="NODE_${1}_IP"; echo "${!_v}"; }
get_node_name() { local _v="NODE_${1}_NAME"; echo "${!_v}"; }
...
process_template() {
    ...
    # Global replacements — bash parameter expansion is used instead of sed to avoid
    # interpreting | & \n and other special chars present in passwords as sed metacharacters.
    local _etcd_ep _etcd_ic
    _etcd_ep=$(get_etcd_endpoints)
    _etcd_ic=$(get_etcd_initial_cluster)
    content="${content//\{\{CLUSTER_NAME\}\}/${CLUSTER_NAME}}"
    content="${content//\{\{PG_VERSION\}\}/${PG_VERSION}}"
    content="${content//\{\{PG_PORT\}\}/${PG_PORT}}"
    content="${content//\{\{PG_DATA_DIR\}\}/${PG_DATA_DIR}}"
    content="${content//\{\{PG_BIN_DIR\}\}/${PG_BIN_DIR}}"
    content="${content//\{\{PG_MAX_CONN\}\}/${PG_MAX_CONN}}"
    content="${content//\{\{PG_SUPERUSER_PASS\}\}/${PG_SUPERUSER_PASS}}"
    content="${content//\{\{PG_REPLICATOR_PASS\}\}/${PG_REPLICATOR_PASS}}"
    content="${content//\{\{PG_ADMIN_PASS\}\}/${PG_ADMIN_PASS}}"
    content="${content//\{\{PG_HBA_SUBNET\}\}/${PG_HBA_SUBNET}}"
    content="${content//\{\{VIP_ADDRESS\}\}/${VIP_ADDRESS}}"
    content="${content//\{\{VIP_NETMASK\}\}/${VIP_NETMASK:-24}}"
    content="${content//\{\{VIP_INTERFACE\}\}/${VIP_INTERFACE}}"
    content="${content//\{\{ETCD_TOKEN\}\}/${ETCD_TOKEN}}"
    content="${content//\{\{ETCD_ENDPOINTS\}\}/${_etcd_ep}}"
    content="${content//\{\{ETCD_INITIAL_CLUSTER\}\}/${_etcd_ic}}"
    content="${content//\{\{DASHBOARD_IP\}\}/${DASHBOARD_IP:-}}"
    content="${content//\{\{PATRONI_API_USER\}\}/${PATRONI_API_USER:-patroni}}"
    content="${content//\{\{PATRONI_API_PASS\}\}/${PATRONI_API_PASS:-}}"
