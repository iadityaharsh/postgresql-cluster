...
# Usage:
#   ./setup-hyperdrive.sh
#   ./setup-hyperdrive.sh --db mydb --user cf_user --hyperdrive-name my-hd
#
# Prerequisites:
#   - Cloudflare Tunnel running (setup-tunnel.sh)
...
if [ "$SKIP_DB_SETUP" = true ]; then
    ...
    sudo -u postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='${HD_USER}'" 2>/dev/null | grep -q 1 && {
        echo "User '${HD_USER}' already exists. Updating password..."
        sudo -u postgres psql -c "ALTER USER ${HD_USER} WITH PASSWORD '${HD_PASS}';" 2>/dev/null
    } || {
        echo "Creating user '${HD_USER}'..."
        sudo -u postgres psql -c "CREATE USER ${HD_USER} WITH PASSWORD '${HD_PASS}';" 2>/dev/null
    }

    # Grant access
    sudo -u postgres psql -c "GRANT ALL PRIVILEGES ON DATABASE ${HD_DB} TO ${HD_USER};" 2>/dev/null
    sudo -u postgres psql -d "$HD_DB" -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${HD_USER};" 2>/dev/null
    sudo -u postgres psql -d "$HD_DB" -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL PRIVILEGES ON TABLES TO ${HD_USER};" 2>/dev/null

    # Add pg_hba entry for SSL connections
    echo "Adding pg_hba entry for SSL connections..."
    PATRONI_CONF=$(find /etc/patroni -name "*.yml" 2>/dev/null | head -1)
    if [ -n "$PATRONI_CONF" ]; then
        # Check if entry already exists
        if ! grep -q "hostssl.*${HD_USER}" "$PATRONI_CONF" 2>/dev/null; then
            patronictl -c "$PATRONI_CONF" edit-config --apply \
                --set "postgresql.pg_hba=[\"hostssl all ${HD_USER} 0.0.0.0/0 md5\"]" \
                2>/dev/null || echo "NOTE: Add 'hostssl all ${HD_USER} 0.0.0.0/0 md5' to pg_hba manually."
        fi
    fi
