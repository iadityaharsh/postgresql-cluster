...
./scripts/cloudflare-hyperdrive.sh
...
- Cloudflare Tunnel running on the cluster (see [Remote Access](cloudflare-tunnel.md))
...
- The backend should call `wrangler hyperdrive create/update/delete` via `child_process.execFile`, similar to how `cloudflare-tunnel.sh` is invoked from `server.js` today
- Use the existing `cloudflare-hyperdrive.sh` script with `--skip-db-setup` flag when the user creates the DB user through the dashboard's SQL interface instead
...
```
./scripts/cloudflare-hyperdrive.sh [options]
...
- [Remote Access setup](cloudflare-tunnel.md) — Tunnel and Access configuration
