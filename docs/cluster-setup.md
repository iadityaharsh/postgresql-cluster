...
**What cluster-setup.sh does on each node:**
...
The cluster-setup.sh orchestrates these scripts automatically, but they can also be run individually if needed:

| Script | Purpose |
|--------|---------|
| `scripts/01-packages.sh` | Install PostgreSQL + etcd + Patroni |
| `scripts/02-etcd.sh` | Configure & start etcd |
| `scripts/03-patroni.sh` | Configure & start Patroni |
| `scripts/04-verify.sh` | Cluster health check |
| `scripts/05-vip.sh` | Floating VIP setup |
| `scripts/cluster-monitor.sh` | Monitoring dashboard (Node.js + React) |
| `scripts/backup-setup.sh` | Borg backup with NFS/SMB storage |
| `scripts/cloudflare-tunnel.sh` | Cloudflare Tunnel for remote access |
| `scripts/backup-run.sh` | Backup cron job |
...
See **[docs/cloudflare-tunnel.md](cloudflare-tunnel.md)** for full setup guide including Access policies, Workers, Hyperdrive, and Pages integration.
...
- **Adding nodes**: Run `configure.sh` again with the new node count, copy to the new node, and run `cluster-setup.sh`.
